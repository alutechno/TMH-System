...
function($scope, $state, $sce, queryService, departmentService, accountTypeService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    var qstring = "select a.id,a.code,a.name,a.description,a.status,b.status_name,a.grade,a.grade_history_start,a.grade_history_end,a.grade_forecast_start,a.grade_forecast_end "+
        " from ref_customer_grade a, "+
    ...
    $scope.fileName = "Customer Grade Reference";
    $scope.exportExcel = function(){

        queryService.post('select code,name,grade,grade_history_start,grade_history_end,grade_forecast_start,grade_forecast_end,description,status_name from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name",'Grade','Grade History Start','Grade History End','Grade Forecast Start','Grade Forecast End', 'Description','Status']);
            //Data
    ...
    $scope.submit = function(){
        ...
        if ($scope.coa.id.length==0){
            ...
            var param = {
                code: $scope.coa.code,
                name: $scope.coa.name,
                grade: $scope.coa.grade,
                grade_history_start: $scope.coa.grade_history_start,
                grade_history_end: $scope.coa.grade_history_end,
                grade_forecast_start: $scope.coa.grade_forecast_start,
                grade_forecast_end: $scope.coa.grade_forecast_end,
            ...
            queryService.post('insert into ref_customer_grade SET ?',param)
            ...
            console.log(param)
            queryService.post('update ref_customer_grade SET ? WHERE id='+$scope.coa.id ,param)
            .then(function (result){
				$scope.disableAction = false;
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.coa.grade = result.data[0].grade
            $scope.coa.grade_history_start = result.data[0].grade_history_start
            $scope.coa.grade_history_end = result.data[0].grade_history_end
            $scope.coa.grade_forecast_start = result.data[0].grade_forecast_start
            $scope.coa.grade_forecast_end = result.data[0].grade_forecast_end
    ...
    $scope.execDelete = function(){
        queryService.post('update ref_customer_grade SET status=\'2\', '+
        ' modified_by='+$localStorage.currentUser.name.id+', ' +
        ' modified_date=\''+globalFunction.currentDate()+'\' ' +
