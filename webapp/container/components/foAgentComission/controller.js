...
function($scope, $state, $sce, queryService, departmentService, accountTypeService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    var qstring = "select a.id,a.code,a.name,a.description,a.notes,a.is_commission,a.status,b.status_name from mst_supplier_agent a, "+
    ...
    $scope.fileName = "Agent Comission Reference";
    ...
    $scope.submit = function(){
        if ($scope.coa.id.length==0){
            ...
            var param = {
                code: $scope.coa.code,
                name: $scope.coa.name,
                is_commission: $scope.selected.is_commission,
            ...
            queryService.post('insert into mst_supplier_agent SET ?',param)
            ...
            console.log(param)
            queryService.post('update mst_supplier_agent SET ? WHERE id='+$scope.coa.id ,param)
            .then(function (result){
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.selected.is_commission = result.data[0].is_commission
    ...
    $scope.execDelete = function(){
        queryService.post('update mst_supplier_agent SET status=\'2\', '+
        ' modified_by='+$localStorage.currentUser.name.id+', ' +
        ' modified_date=\''+globalFunction.currentDate()+'\' ' +
