...
function($scope, $state, $sce, queryService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    $scope.fileName = "Pos Discount";
    $scope.exportExcel = function(){

        queryService.post('select code,name,description,food,beverage,others,status_name from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name",'Description','Food','Beverage','Others','Status']);
            //Data
    ...
    $scope.dtColumns.push(
        ...
        DTColumnBuilder.newColumn('food').withTitle('Food'),
        DTColumnBuilder.newColumn('beverage').withTitle('Beverage'),
        DTColumnBuilder.newColumn('others').withTitle('Others'),
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.field.food = result.data[0].food
            $scope.field.beverage = result.data[0].beverage
            $scope.field.others = result.data[0].others
