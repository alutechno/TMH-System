...
function($scope, $state, $sce, queryService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    $scope.viewtitle = "Banquet Meal Period";
    $scope.table = 'ref_banquet_meal_period'
    ...
    $scope.exportExcel = function(){
        ...
        queryService.post('select code,name,description,sequence_id,time_start,time_end,status_name from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name", 'Description','Sequence','Time Start','Time End','Status']);
            //Data
    ...
    $scope.dtColumns.push(
        ...
        DTColumnBuilder.newColumn('sequence_id').withTitle('Sequence'),
        DTColumnBuilder.newColumn('time_start').withTitle('Time Start'),
        DTColumnBuilder.newColumn('time_end').withTitle('Time End'),
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.field.sequence_id = result.data[0].sequence_id
            $scope.field.time_start = result.data[0].time_start
            $scope.field.time_end = result.data[0].time_end
