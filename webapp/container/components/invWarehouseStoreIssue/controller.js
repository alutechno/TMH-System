    ...
    $scope.submit = function(){
		...
		if ($scope.sr.id.length==0){
            ...
            var param = {
                ...
                required_date: $scope.sr.date,
                request_status: $scope.selected.request_status.selected.id,
				issued_status: $scope.selected.issued_status.selected.id,
    ...
    $scope.child.saveTable = function(sr_id) {
		...
		for (var i =0;i< $scope.items.length; i++) {
            ...
            if (user.isNew && !user.isDeleted){
                        ...
                        if(d1 != d2){
							...
							if(user.issued_qty_n>0){
		                        ...
		                        var sql4 = 'INSERT INTO inv_cost_center_stock(cost_center_id,product_id,stock_qty,stock_qty_l,stock_qty_in_recipe_unit,created_by) '+
		                            ' values('+$scope.selected.cost_center.selected.id+','+user.product_id+',(stock_qty+'+(user.issued_qty_n*user.unit_conversion)+')'+
		                            ',stock_qty_l+'+user.issued_qty_n+', stock_qty_in_recipe_unit+'+(user.issued_qty_n*user.recipe_unit_conversion)+','+$localStorage.currentUser.name.id+') ' +
		                            'ON DUPLICATE KEY UPDATE '+
		                            ' stock_qty = (stock_qty+'+(user.issued_qty_n*user.unit_conversion)+')'+
		                            ' ,stock_qty_l = (stock_qty_l+'+user.issued_qty_n+')'+
				...
				if(sql2!=undefined){
					var sql5=`insert into acc_gl_transaction (code,journal_type_id,sr_id,gl_status,notes,bookkeeping_date,posted_by,created_by)
						values("`+$scope.sr.code+`",15,`+$scope.sr.id+`,1,"`+$scope.sr.request_notes+`",curdate(),`+$localStorage.currentUser.name.id+`,`+$localStorage.currentUser.name.id+`)`
					...
					var sql7=`insert into acc_gl_journal (gl_id,account_id,transc_type,amount,notes,bookkeeping_date)
						values(@id,`+$scope.selected.cost_center.selected.account_id+`,'D',`+amount_tot+`,"`+$scope.sr.request_notes+`",curdate())`
					var sql8=`insert into acc_gl_journal (gl_id,account_id,transc_type,amount,notes,bookkeeping_date)
						values(@id,`+$scope.selected.warehouse.selected.account_id+`,'C',`+amount_tot+`,"`+$scope.sr.request_notes+`",curdate())`
