...
function($scope, $state, $sce, queryService, departmentService, accountTypeService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    var qstring = "select a.id,a.code,a.name,a.description,a.status,b.status_name,a.total_qty,a.borrowed_qty from mst_borrow_item a, "+
    ...
    $scope.fileName = "Borrowed Item";
    $scope.exportExcel = function(){

        queryService.post('select code,name,description,status_name,total_qty,borrowed_qty from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name", 'Description','Status','Total Qty','Borrowed Qty']);
            //Data
    ...
    $scope.dtColumns.push(
        ...
        DTColumnBuilder.newColumn('total_qty').withTitle('Total'),
        DTColumnBuilder.newColumn('borrowed_qty').withTitle('Borrowed'),
    ...
    $scope.submit = function(){
        ...
        if ($scope.coa.id.length==0){
            ...
            var param = {
                ...
                total_qty: $scope.coa.total_qty,
                borrowed_qty: $scope.coa.borrowed_qty,
            ...
            queryService.post('insert into mst_borrow_item SET ?',param)
            ...
            console.log(param)
            queryService.post('update mst_borrow_item SET ? WHERE id='+$scope.coa.id ,param)
            .then(function (result){
				$scope.disableAction = false;
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.coa.total_qty = result.data[0].total_qty
            $scope.coa.borrowed_qty = result.data[0].borrowed_qty
    ...
    $scope.execDelete = function(){
        queryService.post('update mst_borrow_item SET status=\'2\', '+
        ' modified_by='+$localStorage.currentUser.name.id+', ' +
        ' modified_date=\''+globalFunction.currentDate()+'\' ' +
