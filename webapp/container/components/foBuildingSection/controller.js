...
function($scope, $state, $sce, queryService, departmentService, accountTypeService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    var qstring = "select a.id,a.code,a.name,a.description,a.status,b.status_name from mst_building_section a, "+
    ...
    $scope.fileName = "Building Section Reference";
    ...
    $scope.submit = function(){
        ...
        if ($scope.coa.id.length==0){
            ...
            queryService.post('insert into mst_building_section SET ?',param)
            ...
            console.log(param)
            queryService.post('update mst_building_section SET ? WHERE id='+$scope.coa.id ,param)
            .then(function (result){
				$scope.disableAction = false;
    ...
    $scope.execDelete = function(){
        queryService.post('update mst_building_section SET status=\'2\', '+
        ' modified_by='+$localStorage.currentUser.name.id+', ' +
        ' modified_date=\''+globalFunction.currentDate()+'\' ' +
