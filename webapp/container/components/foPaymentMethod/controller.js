...
function($scope, $state, $sce, queryService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    $scope.viewtitle = "Payment Method";
    $scope.table = 'ref_payment_method'

    var qstring = "select a.*,if(is_credit_card = 'Y','Yes','No') as is_credit_card_name,concat(b.code,'-',b.name) as account_name,d.status_name "+
                    "from "+ $scope.table +" a "+
    ...
    $scope.arr.is_credit_card = [
        {id: 'Y', name: 'Yes'},
        {id: 'N', name: 'No'}
    ]

    $scope.arr.account_id = []
    queryService.get('select id,concat(code,\'-\',name) as name from mst_ledger_account where status != 2',undefined)
    .then(function(data){
    ...
    $scope.exportExcel = function(){
        ...
        queryService.post('select code,name,description,credit_limit,if(is_credit_card = "Y","Yes","No") as is_credit_card_name,account_name,status_name from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name", 'Description','Credit Limit','Is Credit Card','Account','Status']);
            //Data
    ...
    $scope.dtColumns.push(
        ...
        DTColumnBuilder.newColumn('is_credit_card_name').withTitle('Is CC'),
    ...
    $scope.submit = function(){
        ...
        if ($scope.field.id.length==0){
            ...
            $scope.field.is_credit_card = $scope.selected.is_credit_card.selected.id;
            $scope.field.account_id = $scope.selected.account_id.selected.id;
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.field.credit_limit = result.data[0].credit_limit
            $scope.field.is_credit_card = result.data[0].is_credit_card
            ...
            for (var i = $scope.arr.is_credit_card.length - 1; i >= 0; i--) {
                if ($scope.arr.is_credit_card[i].id == result.data[0].is_credit_card){
                    $scope.selected.is_credit_card.selected = {name: $scope.arr.is_credit_card[i].name, id: $scope.arr.is_credit_card[i].id}
                }
            }
            for (var i = $scope.arr.account_id.length - 1; i >= 0; i--) {
                if ($scope.arr.account_id[i].id == result.data[0].account_id){
                    $scope.selected.account_id.selected = {name: $scope.arr.account_id[i].name, id: $scope.arr.account_id[i].id}
                }
            }
