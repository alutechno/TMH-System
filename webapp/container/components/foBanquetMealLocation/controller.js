...
function($scope, $state, $sce, queryService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    $scope.viewtitle = "Banquet Meal Location";
    $scope.table = 'ref_banquet_meal_loc'
    ...
    $scope.exportExcel = function(){
        ...
        queryService.post('select code,name,description,max_capacity,status_name from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name", 'Description','Max Capacity','Status']);
            //Data
    ...
    $scope.dtColumns.push(
        ...
        DTColumnBuilder.newColumn('max_capacity').withTitle('Max Capacity'),
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.field.max_capacity = result.data[0].max_capacity
