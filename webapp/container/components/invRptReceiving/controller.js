...
function($scope, $state, $sce, queryService, $localStorage, $compile, $rootScope, globalFunction,API_URL,BIRT_URL) {
    ...
    var qstring = `select * from mst_report_params where group_id=3 order by seq_no`
	$scope.sub={}
	$scope.show={}
	...
	$scope.submit = function(name){
		...
		$scope.urlReport = $sce.trustAsResourceUrl(BIRT_URL+'/frameset?__report='+$scope.sub[name].report_file+url+'&user_id='+$scope.user+"&__overwrite=true&__document="+$scope.sub[name].report_file+"."+(new Date()).getTime()+"&dummyVar="+ (new Date()).getTime())
	}
