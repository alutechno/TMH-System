...
function($scope, $state, $sce, queryService, departmentService, accountTypeService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    var qstring = "select a.id,a.code,a.short_name,a.name,a.description,a.status,b.status_name from ref_hk_item_type a, "+
    ...
    $scope.fileName = "Item Reference Type";
    ...
    $scope.submit = function(){
        if ($scope.coa.id.length==0){
            ...
            queryService.post('insert into ref_hk_item_type SET ?',param)
            ...
            console.log(param)
            queryService.post('update ref_hk_item_type SET ? WHERE id='+$scope.coa.id ,param)
            .then(function (result){
    ...
    $scope.execDelete = function(){
        queryService.post('update ref_hk_item_type SET status=\'2\', '+
        ' modified_by='+$localStorage.currentUser.name.id+', ' +
        ' modified_date=\''+globalFunction.currentDate()+'\' ' +
