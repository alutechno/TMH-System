...
function($scope, $state, $sce, queryService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    $scope.table = 'ref_outlet_menu_group'

    var qstring = "select a.*,b.name as menu_class_name, d.status_name "+
                    "from "+ $scope.table +" a "+
                    "left join ref_outlet_menu_class b on a.menu_class_id = b.id "+
    ...
    $scope.fileName = "Menu_Group";
    $scope.exportExcel = function(){

        queryService.post('select code,name,menu_class_name, description,status_name from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name","Menu Class",'Description','Status']);
            //Data
    ...
    $scope.submit = function(){
        ...
        if ($scope.field.id.length==0){
            ...
            $scope.field.menu_class_id = $scope.selected.menu_class_id.selected.id;
    ...
    $scope.clear = function(){
        ...
        $scope.selected = {
            ...
            menu_class_id: {
                selected: $scope.arr.menu_class_id[0]
            }
        }
    }
