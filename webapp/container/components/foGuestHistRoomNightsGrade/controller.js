...
function($scope, $state, $sce, queryService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    $scope.table = 'ref_hist_night_grade'
    ...
    $scope.fileName = "Guest History Nights Grade";
    $scope.exportExcel = function(){

        queryService.post('select code,name,description,status_name,grade,num_of_night_begin,num_of_night_end from('+qstring + qwhere+')aa order by code',undefined)
        .then(function(data){
            $scope.exportData = [];
            //Header
            $scope.exportData.push(["Code", "Name", 'Description','Status','Grade','Night Begin','Night End']);
            //Data
    ...
    $scope.dtColumns.push(
        ...
        DTColumnBuilder.newColumn('num_of_night_begin').withTitle('Night Begin'),
        DTColumnBuilder.newColumn('num_of_night_begin').withTitle('Night End')
    );
    ...
    $scope.update = function(obj){
        ...
        .then(function(result){
            ...
            $scope.field.grade = result.data[0].grade
            $scope.field.num_of_night_begin = result.data[0].num_of_night_begin
            $scope.field.num_of_night_end = result.data[0].num_of_night_end
