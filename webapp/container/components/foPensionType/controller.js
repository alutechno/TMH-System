...
function($scope, $state, $sce, queryService, departmentService, accountTypeService, DTOptionsBuilder, DTColumnBuilder, $localStorage, $compile, $rootScope, globalFunction,API_URL) {
    ...
    var qstring = "select a.id,a.code,a.name,a.description,a.status,b.status_name from ref_pension a, "+
    ...
    $scope.fileName = "Pension Reference";
    ...
    $scope.submit = function(){
        if ($scope.coa.id.length==0){
            ...
            queryService.post('insert into ref_pension SET ?',param)
            ...
            console.log(param)
            queryService.post('update ref_pension SET ? WHERE id='+$scope.coa.id ,param)
            .then(function (result){
    ...
    $scope.execDelete = function(){
        queryService.post('update ref_pension SET status=\'2\', '+
        ' modified_by='+$localStorage.currentUser.name.id+', ' +
        ' modified_date=\''+globalFunction.currentDate()+'\' ' +
