//TODO ttl
angular.module('app')
.factory('principal', ['$q', '$http', '$timeout', '$localStorage','$rootScope',
function($q, $http, $timeout, $localStorage, $rootScope) {
...
.factory('moduleService', ['$q', '$http', '$timeout', '$localStorage',
function($q, $http, $timeout, $localStorage) {
    return {
        getModule: function(vid) {
            var defer = $q.defer();
            var url= "/api/getModule";
            if (vid){
                url +=  "?id="+vid
            }
...
.factory('roleMenuService', ['$q', '$http', '$timeout', '$localStorage',
function($q, $http, $timeout, $localStorage) {
    return {
        assignMenu: function(vobj) {
            var defer = $q.defer();

            $http.post('/api/assignMenu',vobj)
            .success(function (data, status, headers, config) {
                if (status == '200'){
                    defer.resolve(data);
                }
