...
.factory('productSubCategoryService', ['$q', '$http', '$timeout', '$localStorage', 'API_URL',
function($q, $http, $timeout, $localStorage, API_URL) {
    return {
        get: function(vid) {
            var defer = $q.defer();
            var url= API_URL+"/apiinv/getProductSubCategory";
            if (vid){
                url +=  "?id="+vid
            }
            $http.get(url)
        ...
        create: function(role) {
            var defer = $q.defer();
            $http.post(API_URL+'/apiinv/createProductSubCategory', role)
            .success(function (data, status, headers, config) {
                if (status == '200'){
                    defer.resolve(data);
                }
        ...
        update: function(role) {
            var defer = $q.defer();
            $http.post(API_URL+'/apiinv/updateProductSubCategory',role)
            .success(function (data, status, headers, config) {
                if (status == '200'){
                    defer.resolve(data);
                }
        ...
        delete: function(role) {
            var defer = $q.defer();
            $http.post(API_URL+'/apiinv/deleteProductSubCategory',role)
            .success(function (data, status, headers, config) {
                if (status == '200'){
                    defer.resolve(data);
                }
